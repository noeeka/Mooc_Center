                ...
                <Card>
                    <Form ref="courseForm" class="margin-top-20" :label-width="80">
                        ...
                        <FormItem class="margin-top-20" label="开放时间:" id="time" style="display: none;">
                            <DatePicker type="date"  :value="startTime" format="yyyy-MM-dd" @on-change="getStartTime"  placeholder="选择日期" style="width: 200px"></DatePicker>
                            <span>至</span>
                            <DatePicker type="date"  :value="endTime" format="yyyy-MM-dd" @on-change="getEndTime" placeholder="选择日期" style="width: 200px"></DatePicker>
                        </FormItem>
                        ...
                        <FormItem class="margin-top-20" label="上传视频:">
                            <Button type="primary" @click="handleUploadVideo"><Icon type="ios-film"></Icon>&nbsp;上传视频</Button>
                        </FormItem>
            ...
            getData: function () {
                ...
                this.request({
                    ...
                    if (response.status === 1) {
                        ...
                        that.cover_img = that.img_url = data.cover_img;
                        that.open_status = data.open_status.toString();
                        if (that.open_status === '1') {
                            jq('#time').show();
                            that.startTime = new Date(parseInt(data.start_time) * 1000).toLocaleString().replace(/:\d{1,2}$/, ' ');
                            that.endTime = new Date(parseInt(data.end_time) * 1000).toLocaleString().replace(/:\d{1,2}$/, ' ');
                        }
            ...
            getMyTeacher: function () {
                ...
                function (response) {
                    if (response.status === 1) {
                        ...
                        that.teacher = that.array_map(response.data, 'id', 'nick_name');
