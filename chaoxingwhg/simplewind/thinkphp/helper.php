...
if (!function_exists('simpleQrcode')) {
    /**
     * 创建一个二维码
     * @param array $content 二维码内容
     * @param array $filePath 二维码保存地址
     * @return string 二维码链接
     */
    function simpleQrcode($content , $filePath = null)
    {
        try {
            ...
            $strDate    = date('Ymd');
            $fileSaveName    = (empty($app) ? '' : $app . '/') . $strDate . '/' . md5(uniqid()) . ".png";
            $strSaveFilePath = ROOT_PATH.'public/upload/admin/' . $fileSaveName; //TODO 测试 windows 下

            if (!file_exists(dirname($strSaveFilePath))) {
                mkdir(dirname($strSaveFilePath), 0777, true);
            }

            $qrCode->writeFile($strSaveFilePath);
            return 'admin'.DS.$fileSaveName;
        } catch (Exception $e) {
            throw new LogicException("创建二维码失败");
            return "";
        }
    }
}
