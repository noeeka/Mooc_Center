...
{
    private function in_xuexitong()
    {
        $useragent = $_SERVER['HTTP_USER_AGENT'];
        return strpos($useragent, 'ChaoXingStudy') === false ? false : true;
    }

    private function xuexitong_auto_login()
    {
        $xuexitong_id = cookie('UID');
        if($xuexitong_id > 0){
            passport_auto_login($xuexitong_id);
        }
    }
    ...
    {
        //学习通中自动登陆
        if($this->in_xuexitong()){
            $this->xuexitong_auto_login();
        }
        // 监听home_init
        hook('home_init');
        parent::_initialize();
        ...
        //获取静态文件版本号；
        $version_list = Db::name('version')->column('key,value');
        $this->assign('version_list', $version_list);

        //获取总分管开关
        $switch = Db::name('switch')->value('switch');
