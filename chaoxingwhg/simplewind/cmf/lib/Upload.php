...
//        halt($fileImage);
        ...
        if ($objAsset && $storage['type'] =='Local') {
            ...
            $arrInfo["file_store_key"] = $arrAsset["file_store_key"];
            $arrInfo["local_exist"] = 1;
            $arrInfo['suffix'] = $arrAsset['suffix'];
...
//        }
        ...
        if ($storage['type'] != 'Local') { //  增加存储驱动
            ...
            if ($needUploadToRemoteStorage) {
                ...
                return [
                    ...
                    'url'         => $url,
                    'local_exist' => $arrInfo['local_exist'],
                    'file_key'    => $arrInfo['file_key'],
                    'suffix'      => $arrInfo["suffix"]
                ];
            }
        }
        ...
        return [
            ...
            'file_key'    => $arrInfo['file_key'],
            'suffix'      => $arrInfo["suffix"],
            'file_store_key' => isset($arrInfo["file_store_key"]) ? $arrInfo["file_store_key"]:'',
        ];
    }
