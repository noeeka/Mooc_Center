...
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['url']     = cmf_url('portal1/List/index', ['id' => $item['id']]);
            if( $item['parent_id'] >0){
    ...
    public function addCategory($data)
    {
        ...
        if ($result != false) {
            ...
            if (!empty($data['alias']) && !empty($id)) {
                $routeModel->setRoute($data['alias'], 'portal1/List/index', ['id' => $id], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'portal1/Article/index', ['cid' => $id], 2, 4999);
            }
    ...
    public function editCategory($data)
    {
        ...
        if (empty($oldCategory) || empty($newPath)) {
            ...
            $routeModel = new RouteModel();
            if (!empty($data['alias'])) {
                $routeModel->setRoute($data['alias'], 'portal1/List/index', ['id' => $data['id']], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'portal1/Article/index', ['cid' => $data['id']], 2, 4999);
            } else {
                $routeModel->deleteRoute('portal1/List/index', ['id' => $data['id']]);
                $routeModel->deleteRoute('portal1/Article/index', ['cid' => $data['id']]);
            }
    ...
    public function editMenu($data){
            ...
            $oldMenu = Db::name('AdminMenu')->where('id' , $data['id'])->find();

            Db::name('AdminMenu')->strict(false)->field(true)->update($data);
            $app          = "app";
            $controller   = "controller";
            $action       = $oldMenu["action"];
            $param        = '';
            $authRuleName = "$app/$controller/$action";
            $menuName     = $data["name"];
