...
class Portal extends TagLib
{
    ...
    public function tagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\portal1\service\ApiService::articles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagTagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\portal1\service\ApiService::tagArticles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
<?php
if(!empty({$cid})){
    \$__BREADCRUMB_ITEMS__ = \app\portal1\service\ApiService::breadcrumb({$cid},{$self});
?>
...
parse;
    ...
    public function tagCategories($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\portal1\service\ApiService::categories([
    'where'   => {$where},
    'order'   => '{$order}',
]);
...
parse;
    ...
    public function tagSubCategories($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\portal1\service\ApiService::subCategories({$categoryId});
...
parse;
    ...
    public function tagAllSubCategories($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\portal1\service\ApiService::allSubCategories({$categoryId});
 ?>
<volist name="{$returnVarName}" id="{$item}">
{$content}
</volist>
