...
class MukeController extends AdminBaseController
{
    /**
     * 文章列表
     * @adminMenu(
    ...
    public function index()
    {
        ...
        $haveChild = false;
        if(!isset($param['category']) || $param['category'] == 0 || $param['category'] == 19){
            $param['category'] = 19;
            $haveChild = true;
        }
        $param['venue'] = ['in', UserModel::getCurrentVenue()];
        $data = $postService->adminArticleList($param, $haveChild,0);

        $data->appends($param);

        $categoryTree        = $portalCategoryModel->adminCategoryTree($categoryId, 0, 19);
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            $post   = $data['post'];
            $result = $this->validate($post, 'Muke');
            if ($result !== true) {
                $this->error($result);
            }
            ...
            $this->success('添加成功!', url('Muke/edit', ['id' => $portalPostModel->id]));
        }
