...
use app\admin\model\VenueCommentsModel;
use think\DB;
use app\admin\model\UserModel;


class VenueCommentsController extends AdminBaseController
{
    /*
     * 分类列表
     */
    public function index()
    {
        //获取分类列表
        //1.获取父级评论(集合对象）
        $parentlist = Db::name('venue_comments')->alias('c')->where('level_pid', '=', 0)
            ->order('create_time DESC')->join('venue v', 'c.venueid=v.id', 'left')
            ->join('user u', 'c.userid=u.id')
            ->field('c.*,v.name as title,u.user_nickname')->paginate(10);
        ...
        foreach ($parentArr as $key => $value) {
            $sonlist = Db::name('venue_comments')->alias('c')->where('level_pid', '=', $value['id'])
                ->order('id asc')->join('venue v', 'c.venueid=v.id', 'left')
                ->join('user u', 'c.userid=u.id')
                ->field('c.*,v.name as title,u.user_nickname')->select();
    ...
    public function reply()
    {
        ...
        $comment_info = Db::name('venue_comments')->alias('c')->where('c.id', '=', $id)
            ->order('updatetime DESC')->join('venue v', 'c.venueid=v.id', 'left')
            ->join('user u', 'c.userid=u.id')
            ->field('c.*,v.name as title,u.user_nickname')
            ->find();
    ...
    public function replyDo()
    {
        ...
        $commentModel = new VenueCommentsModel;
        $comment = $commentModel->where('id', $id)->find();

        $data['venueid'] = $comment['venueid'];
        ...
        if ($result) {
          /*  $sys = new SysMessageModel();
            $id = $commentModel->id;
            $sys->publish($id, 1);*/
            $this->success('保存回复成功', 'venue_comments/index');
        } else {
    ...
    public function toggle()
    {
        ...
        $commentsModel = new VenueCommentsModel;
    ...
    public function edit()
    {
        //接收评论id
        $id = $this->request->param('id');
        $array = Db::name('venue_comments')->alias('c')->where('c.id', '=', $id)
            ->order('updatetime DESC')->join('venue v', 'c.venueid=v.id', 'left')
            ->join('user u', 'c.userid=u.id')
            ->field('c.*,v.name as title,u.user_nickname')
            ->find();
        $parentid = $array['parentid'];
        $commer = Db::name('venue_comments')->alias('c')->where('c.id', '=', $parentid)
            ->order('updatetime DESC')->join('venue v', 'c.venueid=v.id', 'left')
            ->join('user u', 'c.userid=u.id')
            ->field('c.*,v.name as title,u.user_nickname')
            ->find();
    ...
    public function editDo()
    {
        ...
        $this->success('修改成功', 'venue_comments/index');
    ...
    public function updateDo()
    {
        ...
        if ($result !== false) {
            $this->success('修改评论成功', 'venue_comments/index');
        } else {
