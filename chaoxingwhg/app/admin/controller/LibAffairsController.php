...
class LibAffairsController extends AdminBaseController
{
    private $allCid = 37;
    /**
     * 文章列表
    ...
    public function index()
    {
        ...
        $haveChild = false;
        if(!isset($param['category']) || $param['category'] == 0 || $param['category'] == $this->allCid){
            $param['category'] = $this->allCid;
            $haveChild = true;
        }
        $data = $postService->adminArticleList($param, $haveChild);

        $data->appends($param);

        $categoryTree = $portalCategoryModel->adminCategoryTree($categoryId, 0, $this->allCid);
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            $post   = $data['post'];
            $result = $this->validate($post, 'LibAffairs');
            if ($result !== true) {
                $this->error($result);
            }
            ...
            $this->success('添加成功!', url('LibAffairs/edit', ['id' => $portalPostModel->id]));
        }
