...
class ArticleController extends Base
{
    ...
    public function index()
    {
        ...
        $has_child = input('param.has_child', 1);
        $vid = input('param.venue', 0);
        $aid = input('param.area', 0);
        $page = input('param.page', 1, 'intval');
        $len = input('param.len', 10, 'intval');
        $field = 'a.id,a.post_title as title,post_content,a.published_time as time,a.more,a.is_top,a.post_hits as hits,a.link,a.post_like as likes,a.post_excerpt as abstract, a.post_source as source';
        ...
        if($has_child == 1){
            ...
            $childIds[] = $cid;
            $where['c.category_id'] = ['in', $childIds];
        }else{
    ...
    function collect_list()
    {
        ...
        if (!empty($articleIds)) {
            $list = $portalPostModel
                ...
                ->page($page, $len)
                ->select();
            $count = $portalPostModel
                ->alias('a')
                ->join('__VENUE__ v', 'a.venue = v.id')
                ->where(['delete_time' => 0, 'a.id' => ['in', $articleIds], 'a.post_status' => 1])
                ->field('a.id,post_title,v.name as venue_name,post_collect as collect_num,a.more,a.published_time')
                ->page($page, $len)->count(1);
            foreach ($list as &$v) {
                $thumb = $v->more;
    ...
    public function read()
    {
        ...
        if ($post == null) {
            ...
            $thumbInDb = isset($post->more['thumbnail']) ? $post->more['thumbnail'] : '';
            $post->thumb = $thumbInDb == '' ? '' : cmf_get_image_preview_url($thumbInDb);
            unset($post->more);
            $post->content = $portalPostModel->getPostContentAttr($post->content);
            ...
            $ids = $portalCatetoryModel->getTreeIds($post->top_cid);
            $post->categorys = (new PortalCategoryModel())
                ->alias('c')
                ->join('__PORTAL_CATEGORY_POST__ p', 'c.id = p.category_id')
                ->where(['p.post_id' => $id, 'p.status' => '1', 'p.category_id' => ['in', $ids]])
                ->field('c.id,c.name')
                ->select();
