...
class Passport extends Base
{
    ...
    public function regist(){
        ...
        {
            $user=new User();
            $login = substr_replace($mobile,'****',3,4);
            $data=[
                'mobile'=>$mobile,
    ...
    public function login(){
        ...
        //获取登录信息
        $user_id        =input('post.user_id');        //用户ID：mobile 或email
        ...
        //验证用户登录信息
        $user=new User();
        $user_info=$user->where($where)->field('id,user_pass,user_status')->find();
