...
class VenuecommentsController extends Base
{
    public function index()
    {
        ...
        $venueid = input('param.venueid', 0, 'intval');
        $comments = new VenueCommentsModel();
        $parent = $comments->alias('a')->where(['a.parentid' => 0, 'a.status' => 1, 'a.venueid' => $venueid])
            ->page($page, $len)
        ...
        $num = $comments->alias('a')->where(['a.parentid' => 0, 'a.status' => 1, 'a.venueid' => $venueid])
            ->join('user u', 'u.id =a.userid')
    ...
    public function comment($param = [])
    {
        $uid = $this->getuid();
        $id = input('parentid', 0);
        $venueid = input('venueid', 0);
        $content = input('content', '');

        if (empty($content)) {
            return $this->output_error(13006, '评论内容不能为空');
        }
        $data = ['venueid' => $venueid, 'userid' => $uid, 'comment' => $content, 'updatetime' => time(), 'create_time' => time(), 'status' => 0];
        $comment = new VenueCommentsModel();
        if ($id == 0) {
      ...
      //  VenueCommentsModel::where(['id' => $venueid])->setInc('comment_count');
