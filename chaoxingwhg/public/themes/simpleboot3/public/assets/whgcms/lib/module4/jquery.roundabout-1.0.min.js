...
jQuery.fn.roundabout = function() {
    ...
    this.each(function(i) {
        ...
        children.each(function(i) {
            ...
            if (options.clickToFocus === true) { jQuery(this).click(function(e) { if (!jQuery.roundabout_isInFocus(ref, degrees)) { e.preventDefault(); if (ref.attr('animating') == '0') { ref.roundabout_animateAngleToFocus(degrees) } return false } }) }
        });
        if (options.btnNext) { jQuery(options.btnNext).click(function(e) { e.preventDefault(); if (ref.attr('animating') == '0') { ref.roundabout_animateToNextChild() } return false }) }
        if (options.btnPrev) { jQuery(options.btnPrev).click(function(e) { e.preventDefault(); if (ref.attr('animating') == '0') { ref.roundabout_animateToPreviousChild() } return false }) } ref.roundabout_updateChildPositions()
    });
    return this
};
...
jQuery.fn.roundabout_animateToBearing = function(bearing) {
    ...
    this.each(function(i) {
        ...
        if (data === null) { data = { timerStart: currentTime, start: jQuery.roundabout_getBearing(ref), totalTime: thisDuration } } timer = currentTime - data.timerStart;
        if (timer < thisDuration) {
            ref.attr('animating', '1');
            if (typeof jQuery.easing.def == 'string') {
                easingFn = jQuery.easing[thisEasingType] || jQuery.easing[jQuery.easing.def];
                newBearing = easingFn(null, timer, data.start, bearing - data.start, data.totalTime)
            } else { newBearing = jQuery.easing[thisEasingType]((timer / data.totalTime), timer, data.start, bearing - data.start, data.totalTime) } ref.roundabout_setBearing(newBearing, function() { ref.roundabout_animateToBearing(bearing, thisDuration, thisEasingType, data) })
        } else {
...
jQuery.fn.roundabout_animateToDelta = function(delta) {
    var duration, easing;
    if (typeof arguments[1] !== 'undefined') { duration = arguments[1] }
    if (typeof arguments[2] !== 'undefined') { easing = arguments[2] } this.each(function(i) {
...
jQuery.fn.roundabout_animateToNextChild = function() {
    var duration, easing;
    if (typeof arguments[0] !== 'undefined') { duration = arguments[0] }
    if (typeof arguments[1] !== 'undefined') { easing = arguments[1] } this.each(function(i) {
        if (jQuery(this).attr('animating', '0')) {
            ...
            var period = parseFloat(jQuery(this).attr('period'), 4),
                j = 0,
                range;
            while (true) { range = { lower: period * j, upper: period * (j + 1) }; if (bearing <= range.upper && bearing > range.lower) { jQuery(this).roundabout_animateToDelta(bearing - range.lower, duration, easing); break } j++ }
        }
    });
    return this
};
    ...
    if (typeof arguments[1] !== 'undefined') { easing = arguments[1] } this.each(function(i) {
        if (jQuery(this).attr('animating', '0')) {
            ...
            var period = parseFloat(jQuery(this).attr('period'), 4),
                j = 0,
                range;
            while (true) { range = { lower: period * j, upper: period * (j + 1) }; if (bearing >= range.lower && bearing < range.upper) { jQuery(this).roundabout_animateToDelta(bearing - range.upper, duration, easing); break } j++ }
        }
    });
    return this
};
jQuery.fn.roundabout_animateAngleToFocus = function(target) {
    ...
    if (typeof arguments[2] !== 'undefined') { easing = arguments[2] } this.each(function(i) {
        ...
        if (delta !== 0) { jQuery(this).roundabout_animateToDelta(delta, duration, easing) }
    });
    return this
};
jQuery.fn.roundabout_updateChildPositions = function() {
    this.each(function(i) {
        ...
        ref.children(ref.attr('childSelector')).each(function(i) {
            ...
            } else { jQuery(this).removeClass('roundabout-in-focus') }
        });
        if (inFocus !== data.inFocus) { jQuery.roundabout_triggerEvent(ref, data.inFocus, 'blur'); if (inFocus !== -1) { jQuery.roundabout_triggerEvent(ref, inFocus, 'focus') } ref.attr('childInFocus', inFocus) }
    });
    return this
};
...
jQuery.roundabout_updateChildPosition = function(child, container, data, childPos) {
    ...
    //修改active图片的宽高
    var itemData = { startWidth: startPos[0]*0.8595, startHeight: startPos[1]*0.765, startFontSize: startPos[2], degrees: parseFloat(ref.attr('degrees'), 4) };
