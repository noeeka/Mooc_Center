    ...
    request({
        ...
        success: function (res) {
            if (res.status == 1) {
                ...
                $('.figure').html(template(res.data.detail));
                //设置标题
                $('title').text(res.data.detail.post_title);
                //收藏按钮样式
                if (res.data.detail.is_collect == 1) {
                    // console.log
                ...
                if (res.data.detail.photo_status == 1) {
                    $('.photos').show();
                    $('.photos .photomore').attr('href', '/wx/volunteer/reportsphotos?id='+res.data.detail.id);
                    res.data.photo = res.data.photo.slice(0, 4);
                    var photoTpl = $('#phote-item').html();
                    var photoTemplate = Handlebars.compile(photoTpl);
                    $('.photos .mui-table-view ').html(photoTemplate(res.data));
                }
            } else {
                $('body').html('<p>内容不存在</p>');
