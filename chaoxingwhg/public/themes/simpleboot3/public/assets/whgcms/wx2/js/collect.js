$(function () {
            ...
            if (itemIndex == 1 ||  itemIndex == 2 ){
                ...
                $.ajax({
                    ...
                    success:function (result) {
                        if(result.status ==1){
                            pg++; //页数加1
                            if (result.data.list.length < len) {
                                // 再往下已经没有数据
                                tab1LoadEnd = true;
                                // 锁定
                                me.lock();
    ...
    //类型
    function collectType(dropload,template) {
        $.ajax({
            ...
            success:function (result) {
                if(result.status ==1){
                    var  date_activity_type=result.data;
                    //类型
                    $("#Sort-Sort").html("");
