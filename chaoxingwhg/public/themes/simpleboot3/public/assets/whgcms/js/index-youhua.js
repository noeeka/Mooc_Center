$(function(){
    ...
    function headerModule(data,templateid){
        var headerModuleTpl = $('#headerModule'+templateid+'-temeplate').html();
        var headerModuleCmp = Handlebars.compile(headerModuleTpl);
        $('#header').html(headerModuleCmp(data))
        if(templateid == 1){
            nav(1150);
    ...
    function bannerModule(data,templateid){
        var bannerModuleTpl = $('#bannerModule'+templateid+'-temeplate').html();
        var bannerModuleCmp = Handlebars.compile(bannerModuleTpl);
        $('#banner').html(bannerModuleCmp(data))
        if(templateid == 4){
            // banner模板1
    ...
    function mainModule(data,templateid){
        ...
        console.log(111);
        var mainModuleTpl = $('#mainModule'+templateid+'-temeplate').html();
        console.log('#mainModule'+templateid+'-temeplate');
        var mainModuleCmp = Handlebars.compile(mainModuleTpl);
        console.log(mainModuleCmp)
        $('#main').append(mainModuleCmp(data));
    ...
    //搜索
    $('body').on('click','.search .sousuo',function() {
        if($('.search .close').css('display') == 'block'){
            var searchval = $('#sousuoinput').val().trim();
            if (searchval == '') {
                return false;
            }
