$(function() {
    ...
    $('.type').show();
    $('.logo').click(function() {
        window.location.href = "/portal1/index";
    })
    //加载网站基本信息
    $.ajax({
    ...
    function nav(navwidth, outerWidth) {
        $.ajax({
            ...
            success: function(res) {
                ...
                var flag = true;
                if (res.status == 1) {
                    if (window.location.href == 'https://' + document.domain + '/portal1/my/index') {
                        var html = '<li><a id="nav0" href="/portal1/index">首页</a></li>';

                    } else {
                        var html = '<li><a id="nav0" class="active" href="/portal1/index">首页</a></li>';
                    }
                    $('.nav').append(html);
    ...
    function search() {
        $('body').on('click', '.sousuo', function() {
            ...
            window.location.href = "/portal1/search/index/?kv=" + searchval;
        });
        //回车键
    ...
    if (token == '') {
        ...
        $('.exit').click(function(event) {
            request({
                ...
                success: function(response) {
                    if (response.status == 1) {
                        if (location.pathname.search('/portal1/my') != -1) {
                            location.href = '/portal1/index';
                            return;
                        }
                        $('.denglu').show();
                        $('.zhuce').hide();
...
function noLogin(res, isRedirectLogin) {
    var regex = /^1000[4-9]$/;
    var url = isRedirectLogin == true ? '/portal1/login/login' : undefined;
