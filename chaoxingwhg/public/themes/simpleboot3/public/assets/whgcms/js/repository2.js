$(function(){
	...
	$('.body').show();
    $('.logo').click(function() {
        window.location.href = "/portal2/index";
    })
    //加载网站基本信息
    $.ajax({
    ...
    function nav(navwidth, outerWidth) {
        $.ajax({
            ...
            success: function(res) {
                ...
                var flag = true;
                if (res.status == 1) {
                    if (window.location.href == 'https://' + document.domain + '/porta2/my/index') {
                        var html = '<li><a id="nav0" href="/portal2/index">首页</a></li>';

                    } else {
                        var html = '<li><a id="nav0" class="active" href="/portal2/index">首页</a></li>';
                    }
                    $('.nav').append(html);
    ...
    function search() {
        $('body').on('click', '.sousuo', function() {
            ...
            window.location.href = "/portal2/search/index/?kv=" + searchval;
        });
        //回车键
        ...
        request({
            ...
            success: function(response) {
                ...
                if (response.status == 1) {
                    // $('.xx').html(picCmp(response.data));
                    if (response.data.avatar != '') {
                        $('.tlogin img').attr('src', response.data.avatar_url);
                        // var len = $('.nav li a').length;
                        // for (var k = 0; k < len; k++) {
                        //     var nav_url = $('.nav li a').eq(k).attr('id');
                        //     if (nav_url == 'nav0' & $('.nav li a').eq(k).hasClass('active')) {
                        //         $('.tlogin img').attr('src', '/themes/simpleboot3/public/assets/whgcms/images/my/whiteavatar.png');
                        //     }
                        // }
                    }
            ...
            request({
                ...
                success: function(response) {
                    if (response.status == 1) {
                        ...
                        //setCookie('token', "", -1);
                        //setCookie('salt', "", -1);
                        if (location.pathname.search('/portal2/my') != -1) {
                            location.href = '/portal2/index';
                            return;
                        }
                        $('.denglu').show();
                        $('.zhuce').hide();
...
function noLogin(res, isRedirectLogin) {
    var regex = /^1000[4-9]$/;
    var url = isRedirectLogin == true ? '/portal2/login/login' : undefined;
