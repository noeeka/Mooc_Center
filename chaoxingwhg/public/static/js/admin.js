...
function openUploadDialog(dialog_title, callback, extra_params, multi, filetype, app) {
    ...
    var openLocalAsset = 0;
    if( arguments[6] == 1 ) openLocalAsset = 1;

    Wind.css('artDialog');
    multi = multi ? 1 : 0;
    filetype = filetype ? filetype : 'image';
    app = app ? app : GV.APP;
    var params = '&multi=' + multi + '&filetype=' + filetype + '&app=' + app +'&openLocalAsset='+openLocalAsset;
    Wind.use("artDialog", "iframeTools", function() {
        art.dialog.open(GV.ROOT + 'user/Asset/webuploader?' + params, {
            ...
            ok: function() {
                if (typeof callback == 'function') {
                    ...
                    var type = iframewindow.get_ImgType();
                    var isUpdate = iframewindow.get_UserSelectMode();
                    if(!isUpdate) {
                        var data = { 'assetType' :type , 'files' : files};
                        updateAsset(data);
                    }
...
function updateAsset(data) {
    ...
    $.post('/api/asset/setAssetType' ,data , function (Rsdata) {
        if(Rsdata.status == 1) {
...
function uploadOne(dialog_title, input_selector, filetype, extra_params, app) {
    ...
    openUploadDialog(dialog_title, function(dialog, files) {
        ...
        console.log("预览地址:" + files[0].preview_url);
 ...
 * @param app  应用名,CMF的应用名
 */
function uploadOneImageAndLocal(dialog_title, input_selector, extra_params, app) {
    openUploadDialog(dialog_title, function(dialog, files) {

        console.log("这里是回调");
        console.log(files);
        console.log("这里是回调结束");
        $(input_selector).val(files[0].weedPath);
        $(input_selector + '-preview').attr('src', files[0].preview_url);
        $(input_selector + '-name').val(files[0].name);
    }, extra_params, 0, 'image', app , 1);
}
